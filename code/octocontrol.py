...
class OctoprintAPI:
    ...
    #     fle={'file': open('test5.gcode', 'rb'), 'filename': 'test5.gcode'}
    #     url= self.base_address + '/api/files/{}'.format('local')
    #     payload={'select': 'true','print': 'false' }
    #     header={'X-Api-Key': 'F5EFDBD8F3AB4C03B8C690B0F6917702' }
    ...
    def fileUpload(self, file):
        ...
        request = requests.post(
            self.base_address+"/api/files/local", headers=self.header, files=fle)
