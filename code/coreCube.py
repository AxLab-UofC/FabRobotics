...
class CoreCube(bluepy.btle.Peripheral):
  #updated for toio firmware v2
  HANDLE_TOIO_ID  = 0x0d
  HANDLE_TOIO_MTR = 0x11
  HANDLE_TOIO_LED = 0x15 #0x14
  HANDLE_TOIO_SND = 0x18 #0x17
  HANDLE_TOIO_SEN = 0x1b #0x1a
  HANDLE_TOIO_BTN = 0x1f #0x1e
  HANDLE_TOIO_BAT = 0x23 #0x22
  HANDLE_TOIO_CFG = 0x27 #0x26
  ...
  def __init__(self):
    ...
    #moveToTarget command#
    self.tx = 220 #
    self.ty = 220 #
    self.tdeg = 0 #
    self.targetingMode = 0 #targetting mode
    self.rotMode = 0 #rotation mode
    self.timeOutSec = 0 #
    self.maxSpeed = 115 #
    self.speedChangeType = 3
  ...
  def toioUpdate(self):
      ...
      if deltaTime > 1./self.cubeUpdateFPS: #update depending on FPS
          if self.mode == 'stop':
              ...
              self.mode ='stop'
          elif self.mode == 'motorToTarget':
              self.motorToTarget(self.tx, self.ty, self.tdeg, self.targetingMode, self.rotMode, self.timeOutSec, self.maxSpeed, self.speedChangeType)
              self.mode ='stop'
  ...
  def toioUpdatePos(self, x_, y_):
      ...
      '''if deltaTime > self.offsetLosttimeSec :
        self.currentCol = 0
        self.currentRow = 0'''
      if xx > 1000: #reset if placed on the RESET BOARD (back of toio mat)
        '''self.currentCol = 0
        self.currentRow = 0
        self.isLost = True'''
      else:
  ...
  def connect(self, deviceAddr, addrType=bluepy.btle.ADDR_TYPE_RANDOM):
      #try:
          ...
          print("CONNECTING   " + deviceAddr)
          bluepy.btle.Peripheral.connect(self, deviceAddr, addrType)
      #except bluepy.btle.BTLEException as e: #does not do anything?
          #print("I think there is an error in connection") #joanne
          #print(e.code)
  ...
  def id(self):
    ...
    idVal = struct.unpack('b', data[0:1])[0]
    if idVal == 0x01:
  ...
  def battery(self):
    ...
    self.batt = struct.unpack('b', data)[0]
    return self.batt

  #  ---------------- Motor Control
  def motor(self, speeds, duration):
    data = "01" if duration == 0 else "02"
    data = data + "01" + ("01" if speeds[0] >= 0 else "02") + ("{:02x}".format(abs(speeds[0])))#Left Motor
    data = data + "02" + ("01" if speeds[1] >= 0 else "02") + ("{:02x}".format(abs(speeds[1])))#Right Motor
    if duration != 0:
      data = data + ("{:02x}".format(duration))
    self.writeCharacteristic(self.HANDLE_TOIO_MTR, binascii.a2b_hex(data))
  #  ---------------- Motor Control
  def motorToTarget(self, control, timeout, moveType, maxSpeed, speedType, tx, ty, trot):
    data = "03{:02x}{:02x}{:02x}{:02x}{:02x}00{:04x}{:04x}{:04x}".format(control, timeout, moveType, maxSpeed, speedType, le(tx), le(ty), le(trot))
    self.writeCharacteristic(self.HANDLE_TOIO_MTR, binascii.a2b_hex(data))
  #  ---------------- Light Control
